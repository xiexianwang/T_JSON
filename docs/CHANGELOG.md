...
- 需求说明书更新至 V1.2，地图方案已由天地图切换为高德 JS API v2.0（见下），增加点选跟踪、雨刷电机等需求。
...
- **地图瓦片源切换到高德地图**：OSM 在境内不可用（超时白屏），迁移至高德 JS API v2.0（`webapi.amap.com`）；添加 WGS-84 → GCJ-02 坐标纠偏；移除 Leaflet 资源引用。
