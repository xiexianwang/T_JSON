                 ...
                 double& outLat, double& outLon)
{
    ...
    double dxAngle = (pixelX - halfW) * pixelSize / (focal * 1000.0);
    double bearingRad = pan * M_PI / 180.0 + dxAngle;
    ...
    double lat2 = qAsin(qSin(lat1) * qCos(d) + qCos(lat1) * qSin(d) * qCos(bearingRad));
    double lon2 = lon1 + qAtan2(qSin(bearingRad) * qSin(d) * qCos(lat1), qCos(d) - qSin(lat1) * qSin(lat2));
                     ...
                     double& outLat, double& outLon)
{
    ...
    double dxAngle = (pixelX - halfW) * pixelSize / (focal * 1000.0);
    double dyAngle = (pixelY - halfH) * pixelSize / (focal * 1000.0);
